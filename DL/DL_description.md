...
#### [1-2] Hidden Layer
<br>
By adding hidden layer composed of several perceptrons to an artificial neural network, the accuracy is much improved.
...
<center>In fact, the most used is `ReLU`.</center>
...
- Gradient Descent : How to find the optimized parameter that minimizes the cost value of the `cost function`<br>
$\theta_{j} =  \theta_{j} - \alpha \cdot \frac {\partial J(\theta_{0}, \theta_{1})}{\partial \theta_{j}}$<br>
$\alpha = Learning Rate$
   - Batch Size : How many data do you have at a time when updating
